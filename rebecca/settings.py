...
PROJECT_MODULE = 'rebecca'

DEBUG = True
TEMPLATE_DEBUG = DEBUG

# For Postgres (not location aware) do from command line
# echo "CREATE USER rebecca WITH PASSWORD 'rebecca'" | sudo -u postgres psql
# echo "CREATE DATABASE rebecca WITH OWNER rebecca ENCODING 'UTF8'" | sudo -u postgres psql

# For Postgres (location aware) do from command line
# echo "CREATE USER rebecca WITH PASSWORD 'rebecca'" | sudo -u postgres psql
# echo "CREATE DATABASE rebecca WITH OWNER rebecca ENCODING 'UTF8' TEMPLATE template_postgis" | sudo -u postgres psql

# For MySQL remember to first do from a MySQL shell:
# CREATE database rebecca;
# GRANT ALL ON rebecca.* TO 'rebecca'@'localhost' IDENTIFIED BY 'rebecca';
# GRANT ALL ON test_rebecca.* TO 'rebecca'@'localhost' IDENTIFIED BY 'rebecca';
# FLUSH PRIVILEGES;

DATABASES = {
    'default': {
        'ENGINE': 'django.contrib.gis.db.backends.spatialite', # Options are spatialite, postgis, mysql.
        'NAME': 'rebecca.db', # Or path to database file if using sqlite3.
        'USER': 'rebecca', # Not used with sqlite3.
        'PASSWORD': 'rebecca', # Not used with sqlite3.
...
SECRET_KEY = '7a40045e5a324d6750c8904c85ceeef73ac911911fae844d3145fe65'
